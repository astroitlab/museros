...
short int ephem_open (double *jd_begin, double *jd_end,
                      short int *de_number)
/*
------------------------------------------------------------------------
...
*/
   ...
   //strncpy(ephem_name,current_absolute_path,strlen(current_absolute_path));
   strcpy(ephem_name,current_absolute_path);
   //if  ((offset = strnstr(ephem_name,"museros",strlen(ephem_name)))==NULL)
   if  ((offset = strstr(ephem_name,"museros"))==NULL)
   {
         printf("***Error in finding current directory***\n");
         exit(-1);
   }
   *(offset+7)='\0';
   //strncat(ephem_name,leapfile,strlen(leapfile));
   strcat(ephem_name,leapfile);

   /* Don't judge the value of EPHFILE */
...
short int planet_ephemeris (double *tjd, short int target,
                            short int center,
