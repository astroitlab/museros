...
class PyFITSIDI:
    config = "museruvfits.xml"
    muser = object()
    Latitude = 42.211833333
    Longitude = 115.2505
    Altitude = 1365.0
    ...
    def checkConfigType(self):
      ...
      if type(self.config) is lxml.etree._ElementTree:
          return self.config
      if type(self.config) is str:
          return etree.parse(self.config)
      if type(self.config) is unicode:
          return etree.parse(self.config)
      else:
          print type(self.config)
          raise

    def parseConfig(self, tagname):
        ...
        xmlData = etree.parse(self.config)
        x = xmlData.getroot()
    ...
    def make_primary(self):
        ...
        params = self.parseConfig('PARAMETERS', self.config)
        primary = self.parseConfig('PRIMARY', self.config)
        common = self.parseConfig('COMMON', self.config)
        common['RDATE'] = self.muser.obs_date
        common['STK_1'] = self.muser.polarization
        common['REF_FREQ'] = self.muser.frequency
    ...
    def make_array_geometry(self, num_rows=1):
        ...
        * DIAMETER: Antenna diameter (optional)
        ...
        # Generate headers from config file
        params = self.parseConfig('PARAMETERS', self.config)
        array_geometry = self.parseConfig('ARRAY_GEOMETRY', self.config)
        common = self.parseConfig('COMMON', self.config)
        common['RDATE'] = self.muser.obs_date
        common['STK_1'] = self.muser.polarization
        common['REF_FREQ'] = self.muser.frequency
    ...
    def make_antenna(self, num_rows=1):
        ...
        * POLCALB:  As above, for feed B
        ...
        # Generate headers from config file
        params = self.parseConfig('PARAMETERS', self.config)
        cards = self.parseConfig('ANTENNA', self.config)
        common = self.parseConfig('COMMON', self.config)
        common['RDATE'] = self.muser.obs_date
        common['STK_1'] = self.muser.polarization
        common['REF_FREQ'] = self.muser.frequency
    ...
    def make_frequency(self,num_rows=1):
        ...
        * BB_CHAN:  ?
        ...
        # Generate headers from config file
        params = self.parseConfig('PARAMETERS', self.config)
        cards = self.parseConfig('FREQUENCY', self.config)
        common = self.parseConfig('COMMON', self.config)
        common['RDATE'] = self.muser.obs_date
        common['STK_1'] = self.muser.polarization
        common['REF_FREQ'] = self.muser.frequency
    ...
    def make_source(self,num_rows=1):
      ...
      num_rows: int
        ...
        # Generate headers from config file
        params = self.parseConfig('PARAMETERS', self.config)
        cards = self.parseConfig('SOURCE', self.config)
        common = self.parseConfig('COMMON', self.config)
        common['RDATE'] = self.muser.obs_date
        common['STK_1'] = self.muser.polarization
        common['REF_FREQ'] = self.muser.frequency
    ...
    def make_uv_data(self, num_rows=1):
        ...
        * FLUX         UV visibility data matrix
        ...
        # Generate headers from config file
        params = self.parseConfig('PARAMETERS', self.config)
        cards = self.parseConfig('UV_DATA', self.config)
        common = self.parseConfig('COMMON', self.config)
        common['RDATE'] = self.muser.obs_date
        cards['DATE-OBS'] = self.muser.obs_date
        common['STK_1'] = self.muser.polarization
        common['REF_FREQ'] = self.muser.frequency

        #common['STK_1']= self.muser.Polarization

        c.append(pf.Column(name='UU', format='1D', \
                           unit='SECONDS', array=np.zeros(num_rows, dtype='float64')))

        c.append(pf.Column(name='VV', format='1D', \
                           unit='SECONDS', array=np.zeros(num_rows, dtype='float64')))

        c.append(pf.Column(name='WW', format='1D', \
                           unit='SECONDS', array=np.zeros(num_rows, dtype='float64')))
    ...
    def make_interferometer_model(self, num_rows=1):
        """
        ...
        * DDISP_1       Rate of change of dispersive del for pol 1 at 1m
        ...
        # Generate headers from config file
        params = self.parseConfig('PARAMETERS', self.config)
        cards = self.parseConfig('INTERFEROMETER_MODEL', self.config)
        common = self.parseConfig('COMMON', self.config)
        common['RDATE'] = self.muser.obs_date
        common['STK_1'] = self.muser.polarization
        common['REF_FREQ'] = self.muser.frequency
    ...
    def make_system_temperature(self, num_rows=1):
        ...
        * TANT_1        Antenna temperatures for polarization 1
        ...
        # Generate headers from config file
        params = self.parseConfig('PARAMETERS', self.config)
        cards = self.parseConfig('SYSTEM_TEMPERATURE', self.config)
        common = self.parseConfig('COMMON', self.config)
        common['RDATE'] = self.muser.obs_date
        common['STK_1'] = self.muser.polarization
        common['REF_FREQ'] = self.muser.frequency
    ...
    def make_gain_curve(self, num_rows=1):
        ...
        * SENS_1      Sensitivities for polarization 1
        ...
        # Generate headers from config file
        params = self.parseConfig('PARAMETERS', self.config)
        cards = self.parseConfig('GAIN_CURVE', self.config)
        common = self.parseConfig('COMMON', self.config)
        common['RDATE'] = self.muser.obs_date
        common['STK_1'] = self.muser.polarization
        common['REF_FREQ'] = self.muser.frequency
    ...
    def make_phase_cal(self, num_rows=1):
        ...
        * PC_RATE_1     phase-cal rates for polarization 1
        ...
        # Generate headers from config file
        params = self.parseConfig('PARAMETERS', self.config)
        cards = self.parseConfig('PHASE_CAL', self.config)
        common = self.parseConfig('COMMON', self.config)
        common['RDATE'] = self.muser.obsdate
        common['STK_1'] = self.muser.Polarization
        common['REF_FREQ'] = self.muser.Frequency
    ...
    def make_flag(self, num_rows=1):
        ...
        * SEVERITY  Severity code
        ...
        # Generate headers from config file
        params = self.parseConfig('PARAMETERS', self.config)
        cards = self.parseConfig('FLAG', self.config)
        common = self.parseConfig('COMMON', self.config)
        common['RDATE'] = self.muser.obs_date
        common['STK_1'] = self.muser.polarization
        common['REF_FREQ'] = self.muser.frequency
    ...
    def make_bandpass(self, num_rows=1):
        ...
        * BIMAG_1:    Bandpass response imaginary component
        ...
        # Generate headers from config file
        params = self.parseConfig('PARAMETERS', self.config)
        cards = self.parseConfig('BANDPASS', self.config)
        common = self.parseConfig('COMMON', self.config)
        common['RDATE'] = self.muser.obsdate
        common['STK_1'] = self.muser.Polarization
        common['REF_FREQ'] = self.muser.Frequency
    ...
    def write_fitsidi(self, fitsidi_file_name):
      ...
      tbl_array_geometry = self.make_array_geometry(num_rows=32)

      tbl_frequency = self.make_frequency(num_rows=1)

      tbl_source = self.make_source(num_rows=1)

      tbl_antenna = self.make_antenna(num_rows=32)

      (t_len, bl_len) = (1,528)
      tbl_uv_data = self.make_uv_data(num_rows=t_len*bl_len)
