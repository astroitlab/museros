...
def cuda_hogbom(gpu_dirty, gpu_dpsf, gpu_cpsf, thresh=0.2, damp=1, gain=0.1, prefix='test'):
    ...
    * prefix: prefix for output image file names
    ...
    t_start = time.time()
    i = 0
    while abs(imax) > (thresh_val) && (i<1000):
        if (np.mod(i, 100) == 0):
...
if __name__ == '__main__':
    ...
    if PLOTME:
        ...
        axs[0].imshow(dpsf, vmin=np.percentile(dpsf, 0), vmax=np.percentile(dpsf, 99), cmap=cm.gray)
        ...
        if pathPrefix == None:
            ...
            dpsf.tofile('test_cleanbeam_axs0.dat')
            cpsf.tofile('test_cleanbeam_axs1.dat')
        else:
            if pathPrefix[-1:] == '/':
                pathPrefix = pathPrefix[:-1]
            plt.savefig(pathPrefix + '/' + 'test_cleanbeam.png')
            dpsf.tofile(pathPrefix+'/'+'test_cleanbeam_axs0.dat')
            cpsf.tofile(pathPrefix+'/'+'test_cleanbeam_axs1.dat')
        plt.close()
    ...
    if PLOTME:
        ...
        if pathPrefix == None:
            ...
            dirty.tofile(prefix+'_dirty_final_axs0.dat')
            (np.roll(np.fliplr(gpu_dirty.get()),1,axis=1)).tofile(prefix+'_dirty_final_axs1.dat')
        else:
            if pathPrefix[-1:] == '/':
               pathPrefix = pathPrefix[:-1]
            plt.savefig(pathPrefix + '/' + prefix + '_dirty_final.png')
            dirty.tofile(pathPrefix+'/'+prefix+'_dirty_final_axs0.dat')
        ...
        if os.getenv("MUSEROS_WORK") == None:
            ...
            dirty.tofile(prefix+'_clean_final_axs0.dat')
            clean.tofile(prefix+'_clean_final_axs1.dat')
        else:
            pathPrefix = os.getenv("MUSEROS_WORK")
            if pathPrefix[-1:] == '/':
               pathPrefix = pathPrefix[:-1]
            plt.savefig(pathPrefix + '/' + prefix + '_clean_final.png')
            dirty.tofile(pathPrefix+'/'+prefix+'_clean_final_axs0.dat')
            clean.tofile(pathPrefix+'/'+prefix+'_clean_final_axs1.dat')
        plt.close()
