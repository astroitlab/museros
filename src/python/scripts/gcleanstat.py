...
def cuda_gridvis(f, settings, plan, chan):
    """
    ...
    if settings['vfile'].find('.fitsidi') != -1:
        ...
        h_uu = np.float64(freq * f[0].data[:].UU)
        h_vv = np.float64(freq * f[0].data[:].VV)
        ...
        for bl in range(0, 780):
            ...
            h_rere[bl] = np.float32(f[0].data[:].data[bl][0][0][chan][0][0])
            h_imim[bl] = np.float32(f[0].data[:].data[bl][0][0][chan][0][1])
            ## make GPU arrays
            h_uu = np.float32(h_uu.ravel())
            h_vv = np.float32(h_vv.ravel())
            gcount = np.int32(np.size(h_uu))
            #gcount = len(gcount.ravel())
            h_rere = np.float32(h_rere.ravel())
            h_imim = np.float32(h_imim.ravel())
        ...
        h_uu = np.float64( freq* f[0].data.par('uu')[good])
        h_vv = np.float64( freq* f[0].data.par('vv')[good])
        gcount = np.int32(np.size(h_uu))
        ## assume data is unpolarized

        h_rere = np.float32(f[0].data.data[good, 0, 0, chan, 0, 0])
        h_imim = np.float32(f[0].data.data[good, 0, 0, chan, 0, 1])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--outfile', help="The output directory", dest='outfile',
                        default="", required=True)
    parser.add_argument('-s', '--size', help="The image size", dest='isize', type=int,
                        default=512, required=True)
    parser.add_argument('-p', '--plot', help="Flag of generating file", dest='plot', type = int,
                        default=1)
    ...
    if infile != None:
        main(infile, outdir, ISIZE, PLOTME)
