...
class Phase:
    def __init__(self, sub_array=None, is_loop_mode=None, obs_date_time=None, frame_number=None, calibration_sequence=None, filename=None, debug=0):
        ...
        if obs_date_time is not None and len(obs_date_time)>0:
            if debug ==1:
                muserlog.info("Searched phase data date and time: %s" % obs_date_time)
    ...
    def Calibration(self):
        if self.frame_number < 0:
            muserlog.error("You should input a positive number!")
            return False
        ...
        if self.debug == 1:
            muserlog.info('Reading Visibility Data of calibration......')
        ...
        for i in range(frame_NUM):
            if (muser_calibration.read_one_frame() == False):  # 32*8bits
                muserlog.error('Cannot read a frame.')
                return False
            ...
            if self.debug == 1:
                muserlog.info("Reading No. %d %s %d %d" % (i, muser_calibration.current_frame_time.get_string(), muser_calibration.sub_band, muser_calibration.polarization))
        ...
        if self.debug == 1:
            muserlog.info("Writing to file: " + os.path.basename(file_name))
        calibration_Data.tofile(file_name)
        if self.debug == 1:
            muserlog.info("Exportphase done.")
        return True
...
def exportphase (
    ...
    try:
        ...
        raise
    	muserlog.post("Failed to export muser phase file", "ERROR")
    return
