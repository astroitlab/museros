...
#from pymuser.MuserFrame import *
#from pymuser.astroCoords import *
...
def serial_clean_beam(dpsf, window=20):
    ...
    h, w = np.shape(dpsf)
    cpsf = np.zeros([h, w])
    cpsf[int(w / 2 - window) : int(w / 2 + window), int(h / 2 - window): int(h / 2 + window)] = dpsf[int(w / 2 - window) : int(w / 2 + window),
                                                                         int(h / 2 - window): int(h / 2 + window)]
    ##Normalize
...
if __name__ == '__main__':
    ...
    # Which example?
    if len(sys.argv) > 1:
        sub_array = sys.argv[1]
    if len(sys.argv) >2:
        example = sys.argv[2]
    if len(sys.argv) > 3:
        ISIZE = float(sys.argv[3])
    else:
        ISIZE = 1024
        # Make plots?
    if len(sys.argv) > 4:
        PLOTME = float(sys.argv[4])
    else:
        PLOTME = 1
